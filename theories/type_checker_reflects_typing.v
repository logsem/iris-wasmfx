...
Proof with auto_rewrite_cond.
  ...
  - destruct topt => //=...
    + (* 2 *)
      ...
      assert (ct_suffix [::CTA_some (T_num T_i32)] [::c; c0] = true) as H; first destruct c0 => //=...
      unfold ct_suffix.
    ...
    + (* 3 *)
      ...
      { unfold select_return_top.
        ...
        - destruct (List.nth_error ts (length ts-2)) eqn:Hts1; last by apply List.nth_error_None in Hts1; rewrite length_is_size in Hts1; lias.
          ...
          apply nth_error_ssr with (x0 := v0) in Htype.
          repeat rewrite length_is_size in Htype.
          assert (nth (CTA_some v0) (to_ct_list ts) (size ts - 3) = CTA_some v0) as Hts3ssr.
          { unfold to_ct_list. rewrite -> nth_map with (x1 := v0); last by lias. by rewrite Htype. }
          ...
          apply nth_error_ssr with (x0 := v0) in Hts1.
          repeat rewrite length_is_size in Hts1.
          assert (nth (CTA_some v0) (to_ct_list ts) (size ts - 2) = CTA_some v0) as Hts2ssr.
          { unfold to_ct_list. rewrite -> nth_map with (x1 := v0); last by lias. by rewrite Hts1. }
          ...
          simpl in *.
          move/eqP in Hl3; subst. destruct v0 => //.
          by eexists. 
        - destruct (List.nth_error ts (length ts-2)) eqn:Hts1; last by apply List.nth_error_None in Hts1; rewrite length_is_size in Hts1; lias.
          symmetry in Htype.
          ...
          2: { rewrite length_is_size; unfold to_ct_list; rewrite size_map; apply Hts2ssr. }
          simpl in *.
          move/eqP in Hl2; subst. destruct v0 => //.
          by eexists. 
...
Lemma isolate_prefix_spec whole suffix result :
  isolate_prefix whole suffix = Some result -> whole = result ++ suffix.
Proof.
  unfold isolate_prefix.
  destruct (_ <= _) => //.
  destruct (drop (length whole - length suffix) whole == suffix) eqn:Hdrop => //.
  move/eqP in Hdrop. intros H. rewrite - Hdrop.
  inversion H; subst.
  rewrite cat_take_drop.
  done.
Qed.

Lemma drop_all {A} (l1 : list A) l2 :
...
Lemma isolate_prefix_trivial ts t1s :
...
Proof.
  ...
  rewrite H.
  rewrite drop_all eq_refl take_all. done.
Qed. 
  ...
  - move:Hct.
    ...
    + rewrite size_rcons.
      ...
      * move => Hct.
        ...
        -- apply ct_suffix_1_impl in Hct.
           destruct Hct as [v [tm' ?]]; subst.
           exists (tm' ++ [::v; v; T_num T_i32]); split; first by apply ct_suffix_any_1; rewrite size_ct_list size_cat; lias.
           apply bet_weakening.
           by apply bet_select.
        -- done.
        -- destruct v => //=.
           ...
           apply ct_suffix_1_impl in Hct.
           destruct Hct as [v [tm' ?]]; subst.
           exists (tm' ++ [::v; v; T_num T_i32]); split.
           ++ unfold ct_suffix...
              apply/andP; split; first by lias.
              unfold to_ct_list.
        ...
        }
        {
          ...
          - repeat rewrite length_is_size size_cat in Hct.
            ...
            + replace (_+_-_) with (size l) in Hct; last by lias.
              ...
              split; last by rewrite - cats1; apply bet_weakening; apply bet_select.
              destruct c , c0 => //=; auto_rewrite_cond; unfold to_ct_list in Hct; rewrite map_rcons in Hct; (try rewrite cats1 in Hct); apply ct_suffix_rcons in Hct; destruct Hct; unfold to_ct_list; rewrite map_cat; apply ct_suffix_append_compat => //=...
          - unfold ct_suffix in *; destruct l; auto_rewrite_cond; try by lias.
...
Lemma drop_app {A} n (l1: list A) l2 :
  n <= size l1 -> drop n (l1 ++ l2) = drop n l1 ++ l2.
Proof.
  move:n. induction l1; intros n => //=.
  - destruct n => //. rewrite drop0 => //.
  - destruct n; first by rewrite drop0.
    simpl. intros Hn; apply IHl1. lias.
Qed. 

Lemma map_cat_inv {A B} l l1 l2 (f : A -> B):
  map f l = l1 ++ l2 -> exists l1' l2', l = l1' ++ l2' /\ map f l1' = l1 /\ map f l2' = l2.
Proof.
  move:l. induction l1; intros l => //=.
  - intros H. exists [::], l. repeat split => //.
  - destruct l => //.
    intros H; inversion H.
    apply IHl1 in H2 as (l1' & l2' & -> & Hl1 & Hl2).
    exists (a0 :: l1'), l2'. repeat split => //. simpl. by rewrite Hl1.
Qed.

Lemma ct_list_compat_cat_inv l1 l2 l1' l2':
  ct_list_compat (l1 ++ l2) (l1' ++ l2') -> size l2 = size l2' ->
  ct_list_compat l1 l1' /\ ct_list_compat l2 l2'.
Proof.
  move:l1'. induction l1; intros l1' => //.
  - destruct l1' => //.
    intros H Hsize.
    apply all2_size in H.
    repeat rewrite size_cat in H. simpl in H. lias.
  - destruct l1' => //.
    + intros H Hsize.
      apply all2_size in H.
      repeat rewrite size_cat in H. simpl in H. lias.
    + simpl. intros H Hsize.
      move/andP in H.
      destruct H as [-> Hcompat].
      simpl.
      apply IHl1 => //.
Qed.
...
Lemma ct_suffix_cat s1 s2 l1 l2:
  ct_suffix (s1 ++ s2) (l1 ++ l2) -> size s2 = size l2 ->
  ct_suffix s1 l1 /\ ct_list_compat s2 l2.
Proof.
  intros Hsuff Hlen.
  unfold ct_suffix in Hsuff.
  move/andP in Hsuff. destruct Hsuff as [Hsize Hcompat].
  do 2 rewrite size_cat in Hsize.
  do 2 rewrite size_cat in Hcompat.
  replace (size l1 + size l2 - (size s1 + size s2)) with (size l1 - size s1) in Hcompat;
    last lias.
  rewrite drop_app in Hcompat; last lias.
  apply ct_list_compat_cat_inv in Hcompat as [Hcompatl Hcompatr] => //.
  split; last by apply ct_list_compat_symmetry.
  apply/andP.
...
Proof with auto_rewrite_cond.
  ...
  - destruct e => //=; (try destruct f as [tn' tm' (* | tn' tm' *)]); auto_rewrite_cond; move => ? Hs Hct Hct2; simplify_type_update => //...
    ...
    + (* Br_table *)
      ...
      apply tc_to_bet_br in Hconsume.
      destruct Hconsume as [tn Hcts].
      exists (tn ++ l0 ++ [::T_num T_i32]); split => //.
      apply bet_br_table.
    ...
    + (* Ref_is_null *)
      ...
      destruct cts => //.
      * destruct (ct_suffix _ _) eqn:Hsuffix => //.
        -- simpl in Hct2. simpl.
           unfold ct_suffix in Hsuffix.
           move/andP in Hsuffix. destruct Hsuffix as [Hsizel Hcompat].
           simpl in Hsizel.
           assert (exists l' x, l = l' ++ [:: x]) as (l' & x & ->).
           { remember (drop (size l - 1) l) as sing.
             assert (size sing = 1); 
               first by subst; rewrite size_drop; lias.
             do 2 destruct sing => //.
             exists (take (size l - 1) l), c.
             rewrite Heqsing cat_take_drop.
             done. }
           rewrite size_cat in Hcompat.
           simpl in Hcompat.
           replace (size l' + 1 - 1) with (size l') in Hcompat; last by lias.
           rewrite drop_all in Hcompat.
           simpl in Hcompat. move/andP in Hcompat.
           destruct Hcompat as [Hcompat _].
           rewrite size_cat in Hct2. simpl in Hct2.
           ...
           { unfold ct_suffix in Hct2.
             move/andP in Hct2.
             destruct Hct2 as [Hsize Hcompat2].
             rewrite size_cat in Hsize. simpl in Hsize.
             apply ct_list_compat_cat2 in Hcompat2 as [Hcompatl Hcompatr].
             rewrite drop_drop size_cat in Hcompatr.
             simpl in Hcompatr.
             replace (size l' + (size lm - (size l' + 1)))
               with (size lm - 1) in Hcompatr; last by lias.
             destruct (drop (size lm - 1) lm) eqn:Hdrop => //.
             move/andP in Hcompatr.
             destruct Hcompatr as [Hc Hrest].
             destruct l => //.
             rewrite size_cat in Hcompatl. simpl in Hcompatl.
             
             exists (take (size lm - (size l' + 1)) lm),
               (take (size l') (drop (size lm - (size l' + 1)) lm)), c.
             repeat split => //.
             rewrite - Hdrop.
             rewrite take_drop.
             remember (size lm - (size l' + 1)) as n.
             replace (take n lm) with (take n (take (size l' + n) lm));
               last by rewrite take_takel => //; lias.
             rewrite catA cat_take_drop.
             subst n.
             replace (size l' + (size lm - (size l' + 1))) with (size lm - 1); last lias.
             by rewrite cat_take_drop.
           }
           rewrite Hlm in Heqlm.
           symmetry in Heqlm.
           apply map_cat_inv in Heqlm as (bef' & aft' & -> & Hbef' & Haft').
           apply map_cat_inv in Haft' as (l''' & c' & -> & Hl''' & Hc').
           destruct c' => //. destruct c' => //.
           simpl in Hc'. inversion Hc'; subst c.
           rewrite Hlm in Hct2.
           rewrite catA in Hct2.
           apply ct_suffix_cat in Hct2 as [Hsuff Hv]; last done.
           unfold ct_list_compat in Hv.
           move/andP in Hv. destruct Hv as [Hv _].
           unfold ct_compat in Hv.
           move/eqP in Hv. subst v.
           exists (bef' ++ l''' ++ [:: populate_ct_aux_single x]).
           split.
           ++ unfold to_ct_list. repeat rewrite map_cat.
              apply ct_suffix_extend.
              apply ct_suffix_append_compat.
              ** subst lm.
                 rewrite Hl'''.
                 unfold ct_suffix. apply all2_size in Hl' as Hsizel'.
                 rewrite Hsizel'. rewrite leqnn => /=.
                 replace (size l' - size l') with 0; last lias.
                 rewrite drop0. done.
              ** simpl. destruct x => //. destruct v => //.
                 unfold populate_ct_aux_single. unfold ct_compat.
                 apply/andP. split => //.
           ++ do 2 apply bet_weakening.
              destruct x => //=; try constructor.
              destruct v => //. constructor.
        -- destruct (ct_suffix l _) eqn:Hsuffix' => //.
           simpl in Hct2.
           assert (exists tm', tm = tm' ++ [:: T_num T_i32]) as (l' & ->).
           { remember (drop (size tm - 1) tm) as sing.
             assert (size sing = 1).
             { subst; rewrite size_drop. destruct tm => //=. lias. } 
             do 2 destruct sing => //.
             assert (tm = (take (size tm - 1) tm) ++ [:: v]) as Htm.
             { rewrite Heqsing cat_take_drop => //. }
             rewrite Htm in Hct2.
             unfold to_ct_list in Hct2. rewrite map_cat in Hct2.
             move/andP in Hct2.
             destruct Hct2 as [_ Hct2].
             rewrite size_cat in Hct2.
             simpl in Hct2.
             assert (size [seq CTA_some i | i <- take (size tm - 1) tm] + 1 - 1 =
                       size [seq CTA_some i | i <- take (size tm - 1) tm]) as Heq; first lias.
             rewrite Heq in Hct2.
             rewrite drop_all in Hct2.
             move/andP in Hct2.
             destruct Hct2 as [Hcompat _].
             destruct v => //. destruct n => //. 
             exists (take (size tm - 1) tm).
             exact Htm. 
           }
           destruct l.
           ++ exists (l' ++ [:: T_ref T_exnref]).
              ...
              ** apply bet_weakening.
                 constructor.
           ++ move/andP in Hsuffix'. destruct Hsuffix' as [Hsize Hcompat].
              simpl in Hsize. destruct l => //.
              simpl in Hcompat. destruct c => //. destruct v => //.
      * simpl. exists l; split; first done.
        destruct (ct_suffix _ _) eqn:Hsuffix => //.
        simpl in Hct2. move/eqP in Hct2.
        rewrite - (cat_take_drop (size l - 1) l). rewrite - Hct2.
        apply bet_weakening.
        move/andP in Hsuffix.
        destruct Hsuffix as [Hsize Hcompat].
        simpl in Hcompat.
        rewrite size_map in Hcompat.
        rewrite - map_drop in Hcompat.
        remember (drop (size l - 1) l) as sing.
        destruct sing => //.
    ...
    + (* Contbind *)
      ...
      destruct (l2 == l0) eqn:Hl2 => //.
      move/eqP in Hl2. subst.
      apply type_update_type_agree in Hct2 as (tn' & Hct & ->).
...
Proof with auto_rewrite_cond.
  move => C bes tf.
  destruct tf as [tn tm (* | tn tm *) ] => //.
  (* 2:{ simpl. constructor. intros Habs. inversion Habs. } *)
