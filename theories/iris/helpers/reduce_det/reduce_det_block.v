...
Lemma block_det vs n m t1s t2s s f es s' f' es' :
  const_list vs ->
  length vs = n ->
  length t1s = n ->
  length t2s = m ->
  reduce s f (vs ++ [AI_basic (BI_block (Tf t1s t2s) es)]) s' f' es' ->
  reduce_det_goal s f [AI_label m [] (vs ++ to_e_list es)] s' f' es' (vs ++ [AI_basic (BI_block (Tf t1s t2s) es)]).
Proof.
