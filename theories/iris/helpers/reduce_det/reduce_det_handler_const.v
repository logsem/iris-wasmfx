...
Lemma handler_const_det es vs s f s' f' es':
  const_list vs ->
  reduce s f [AI_handler es vs] s' f' es' ->
  reduce_det_goal s f vs s' f' es' [AI_handler es vs]. 
Proof.
  intros Hvs Hred.
  lazymatch goal with
...
Lemma handler_trap_det es s f s' f' es':
  reduce s f [AI_handler es [AI_trap]] s' f' es' ->
  reduce_det_goal s f [AI_trap] s' f' es' [AI_handler es [AI_trap]]. 
Proof.
  intros Hred.
  lazymatch goal with
  | _ : reduce _ _ ?es _ _ _ |- _ => remember es as ves
  ...
  - repeat split => //. left => //.  
  - inversion Heqves'; subst.
    apply hfilled_singleton in H as [??] => //.
  - inversion Heqves'; subst.
    apply hfilled_singleton in H as [??] => //. 
  - move/lfilledP in H; inversion H; subst.
    all: try by do 4 destruct vs => //. 
