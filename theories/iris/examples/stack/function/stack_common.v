...
Proof.
  ...
  simpl Z.to_N. 
  apply N.Lcm0.mod_divide. 
  rewrite N.Div0.mul_mod_distr_r;try lia.
  rewrite N.Div0.mul_mod_idemp_l;try lia.
  rewrite N.Div0.mod_mul;lia.
Qed.
...
Proof.
  ...
  - iApply wp_seq.
    ...
    apply N.Lcm0.mod_divide.
    apply Z2N.inj_iff in e; try lia.
    rewrite Z2N.inj_mod in e; try lia.
    rewrite N2Z.id in e. done.
...
Proof.
  ...
  - apply N.ltb_ge in Hlen. simpl in *. iDestruct ("Hstack" with "[]") as "Hstack".
    { iPureIntro. lias. }
...
Proof.
  intros k. iIntros "(%Hinst & %Hlocs & %Hlen & Hf & Hlen)".
  simpl.
  rewrite separate1.
  iApply wp_seq.
  instantiate (1 := λ x, (⌜ x = immV [value_of_uint v] ⌝ ∗ ↪[frame] f)%I).
