...
Definition ewp_pre `{!irisGS wasm_lang Σ} :
  (coPset -d> expr -d> (tagidx -d> iProt Σ) -d> (tagidx -d> iProt Σ) -d> (tagidx -d> val -d> iPropO Σ) -d> (val -d> iPropO Σ) -d> iPropO Σ) →
  (coPset -d> expr -d> (tagidx -d> iProt Σ) -d> (tagidx -d> iProt Σ) -d> (tagidx -d> val -d> iPropO Σ) -d> (val -d> iPropO Σ) -d> iPropO Σ)
  ...
  | None =>
      match to_eff e₁ with
      | Some (thrE vs i a sh) =>
          Ψeff a (immV vs) (* Hmmm have value instead of val to avoid immV? *)
      | Some (susE vs i sh) =>
          iProt_car (upcl (Ψsus i)) (immV vs) (λ w, ▷ ewp E (susfill i sh (of_val w)) Ψsus Ψsw Ψeff Φ)
      | Some (swE vs k tf i sh) =>
          iProt_car (upcl (Ψsw i)) (immV vs) (λ w, ▷ ewp E (swfill i sh (of_val w)) Ψsus Ψsw Ψeff Φ)
      | None =>
          ∀ σ₁ ns κ κs nt,
...
Proof.
  rewrite /ewp_pre=> n ewp ewp' Hwp E e Ψsus Ψsw Ψeff Φ.
...
Lemma ewp_unfold `{!irisGS wasm_lang Σ} E e Ψ1 Ψ2 Ψ3 Φ :
  ewp_def E e Ψ1 Ψ2 Ψ3 Φ ⊣⊢ ewp_pre ewp_def E e Ψ1 Ψ2 Ψ3 Φ.
Proof. by rewrite /ewp_def; apply (fixpoint_unfold ewp_pre). Qed.

Global Instance ewp_ne `{!irisGS wasm_lang Σ} E e n :
  Proper ((dist n) ==> (dist n) ==> (dist n) ==> (dist n) ==> (dist n)) (ewp_def E e).
Proof.
  revert e.
  induction (lt_wf n) as [n _ IH]=> e Ψ1 Ψ1' HΨ1 Ψ2 Ψ2' HΨ2 Ψ3 Ψ3' HΨ3 Φ Φ' HΦ.
  ...
  - f_equiv.
    ...
    + f_equiv. f_equiv. f_equiv.
      * f_equiv. apply IProt_ne. f_equiv. apply HΨ2.
      * f_equiv. intros ?. do 2 (f_contractive || f_equiv).
        apply IH; try lia; eapply dist_le; eauto with lia.
    + f_equiv.
...
Notation "'EWP' e @ E {{ Φ } }" :=
  (ewp_def E e%E (λ _, iProt_bottom) (λ _, iProt_bottom) (λ _ _, False%I) Φ)
  (at level 20, e, Φ at level 200,
   format "'[' 'EWP'  e  '/' '[          ' @  E  {{  Φ  } } ']' ']'") : bi_scope.
Notation "'EWP' e @ E <| Ψ '|' '>'  {{ Φ } }" :=
  (ewp_def E e%E Ψ (λ _, iProt_bottom) (λ _ _, False%I) Φ)
...
Notation "'EWP' e @ E {{ v , Φ } }" :=
  (ewp_def E e%E (λ _, iProt_bottom) (λ _, iProt_bottom) (λ _ _, False%I) (λ v, Φ))
  (at level 20, e, Φ at level 200,
   format "'[' 'EWP'  e  '/' '[          ' @  E  {{ v , Φ  } } ']' ']'") : bi_scope.
Notation "'EWP' e @ E <| Ψ '|' '>'  {{ v , Φ } }" :=
  (ewp_def E e%E Ψ (λ _, iProt_bottom) (λ _ _, False%I) (λ v, Φ))
  (at level 20, e, Ψ, Φ at level 200,
...
Notation "'EWP' e {{ v , Φ } }" :=
  (ewp_def ⊤ e%E (λ _, iProt_bottom) (λ _, iProt_bottom) (λ _ _, False%I) (λ v, Φ))
  (at level 20, e, Φ at level 200,
   format "'[' 'EWP'  e  '/' '[          ' {{ v , Φ  } } ']' ']'") : bi_scope.
Notation "'EWP' e <| Ψ '|' '>'  {{ v , Φ } }" :=
  (ewp_def ⊤ e%E Ψ (λ _, iProt_bottom) (λ _ _, False%I) (λ v, Φ))
  (at level 20, e, Ψ, Φ at level 200,
...
Notation "'EWP' e {{ Φ } }" :=
  (ewp_def ⊤ e%E (λ _, iProt_bottom) (λ _, iProt_bottom) (λ _ _, False%I) Φ)
  (at level 20, e, Φ at level 200,
   format "'[' 'EWP'  e  '/' '[          '  {{  Φ  } } ']' ']'") : bi_scope.
Notation "'EWP' e <| Ψ '|' '>'  {{ Φ } }" :=
  (ewp_def ⊤ e%E Ψ (λ _, iProt_bottom) (λ _ _, False%I) Φ)
  (at level 20, e, Ψ, Φ at level 200,
...
(* Tests *)
(* Check (λ Σ es P Q, EWP es <| λ _, iProt_bottom |> <{ λ i v, ⌜ i = Mk_tagidx 0 ⌝ }> {{ λ v, True }})%I.
Check (λ Σ es P Q, EWP es <| λ i w, λne Φ, ⌜ i = Mk_tagidx 1 ⌝ ∗ ((>> v >> ! v {{ P }} ; << w << ? w {{ Q }})%iprot w Φ) |> <{ λ i v, ⌜ i = Mk_tagidx 0 ⌝ }> {{ λ v, True }})%I. *)
