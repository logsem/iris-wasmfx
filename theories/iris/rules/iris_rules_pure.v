...
Proof.
  ...
  - iPureIntro.
    ...
    eexists [], [AI_basic (BI_const v')], (_,_,_), [].
...
Proof.
  ...
  - iPureIntro.
    unfold reducible, language.prim_step => /=.
    eexists [], [AI_basic (BI_const (VAL_int32 (wasm_bool b)))], (_,_,_), [].
    unfold iris.prim_step => /=.
    repeat split => //.
    apply r_simple.
    subst.
...
Proof.
  ...
  - iPureIntro.
    unfold language.reducible, language.prim_step => /=.
    exists [], [AI_const v2], (σ, f_locs f0, f_inst f0), [].
...
Proof.
  ...
  - iPureIntro. unfold language.reducible, language.prim_step => /=.
    exists [], [AI_const v1], (σ, f_locs f0, f_inst f0), [].
    unfold iris.prim_step => /=. repeat split => //.
