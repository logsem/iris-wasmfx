  ...
  - rewrite to_val_cons_None => //.
    ...
    + rewrite to_eff_cons_None => //.
      ...
      * iIntros (es2 s2 inst2 locs2 HStep).
        ...
        2:{ instantiate (2 := (_,_,_)). exact HStep. } 
        destruct H1 as [[-> HStep2] | [lh1 [lh2 [Hlf1 [Hlf2 Hσ]]]]].
        -- iSpecialize ("H" $! (drop 1 es2) s2 _ _ HStep2).
           iMod "H".
           repeat iModIntro.
           repeat iMod "H".
           iModIntro.
           ...
           iApply "IH".
           by iFrame.
        -- eassert (iris.prim_step es _ [] [AI_trap] _ []) as HStep2.
           ...
           iSpecialize ("H" $! [AI_trap] _ _ _ HStep2).
           iMod "H".
           repeat iModIntro.
           repeat iMod "H".
           iDestruct "H" as "(Hσ & H)".
...
(*         
  ...
  - (* Ind *)
    ...
    + iSpecialize ("Hes1" $! σ with "[$]").
      ...
      * iIntros (e2 s2 inst2 locs2 HStep).
        apply (prim_step_split_reduce_r es1 es2 e2 (σ, f_locs f, f_inst f) (s2, locs2, inst2)) in HStep ; last by [].
        destruct HStep as [[es'' [-> HStep]] | [n [m [lh [Hlf1 [Hlf2 Hσ]]]]]]; last inversion Hσ; subst.
        -- iSpecialize ("H2" $! es'' s2 inst2 locs2 HStep).
           iMod "H2".
           repeat iModIntro.
           repeat iMod "H2".
           iModIntro.
        ...
        -- move/lfilledP in Hlf1.
           ...
           eassert (iris.prim_step es1 (σ, _,_) [] [AI_trap] (σ, _,_) []) as HStep2.
           { unfold iris.prim_step.
             repeat split => //.
             apply r_simple; eapply rs_trap => //.
             move => HContra; subst.
  ...
  destruct (to_val LI) as [vs|] eqn:Hetov;
   ...
   - repeat rewrite ewp_unfold. rewrite /ewp_pre /=.
     ...
     + iSpecialize ("Hes1" $! σ with "[$]").
       ...
       * iIntros (e2 σ2 ? ? HStep').
         eapply (lfilled_prim_step_split_reduce_r _ _ _ _ (_,_,_) _ _ (_,_,_)) in HStep' as Heq;[|apply Hfilled|apply H1].
         destruct Heq as [[e' [HStep'' Hlfilled']] | [[lh' Hlf] Hσ]]; last inversion Hσ; subst.
         -- iSpecialize ("H2" $! e' σ2 _ _ HStep'').
            iMod "H2".
            repeat iModIntro.
            repeat iMod "H2".
            iModIntro.
         ...
         -- eassert (iris.prim_step es1 (_,_,_) [] [AI_trap] (_,_,_) []) as HStep2.
            { unfold iris.prim_step.
              repeat split => //.
              apply r_simple; eapply rs_trap => //.
              move => HContra; subst.
